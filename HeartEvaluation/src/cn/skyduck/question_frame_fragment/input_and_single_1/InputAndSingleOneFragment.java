...
public class InputAndSingleOneFragment extends Fragment {
	...
	// 用户所做答案 - 数据源
	private InputAndSingleOneUserAnswerDataSource userAnswerDataSource;

	// 跟 AnswerActivity 通信的接口
	private IQuestionnaireFragmentCallback questionnaireFragmentCallback;

	//
	private InputAndSingleOneFragmentFactoryMethod.FragmentStyleEnum fragmentStyleEnum = InputAndSingleOneFragmentFactoryMethod.FragmentStyleEnum.vertical;
	//
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		final View rootView = inflater.inflate(R.layout.fragment_input_and_single_one, container, false);
		...
		if (!TextUtils.isEmpty(userAnswerDataSource.getInputBoxContent())) {
			inputEditText.setText(userAnswerDataSource.getInputBoxContent());
		}
		...
		okButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				userAnswerDataSource.setInputBoxContent(inputEditText.getText().toString());
		...
		case horizontal:// 水平
			...
			heightOfSpaceView = (int) ToolsFunctionForThisProgect.getRawSize(TypedValue.COMPLEX_UNIT_DIP, 150);
		...
		for (int i = 0; i < anwserList.size(); i++) {
			...
			radioButton.setOnClickListener(new View.OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					userAnswerDataSource.setSingleAnswerOption(v.getId());
					okButton.setEnabled(true);
					inputEditText.setEnabled(true);
				}
			});

			if (userAnswerDataSource.getSingleAnswerOption() == i) {
	...
	public void onAttach(Activity activity) {
		...
		if (this.questionFragmentDataSource != null) {
			...
			if (questionFragmentDataSource.getAnswerDataSource() == null) {
				this.userAnswerDataSource = new InputAndSingleOneUserAnswerDataSource();
			} else {
				this.userAnswerDataSource = (InputAndSingleOneUserAnswerDataSource) questionFragmentDataSource.getAnswerDataSource();
			}

			//
			if (this.questionFragmentDataSource.getFragmentStyle() != null) {
				this.fragmentStyleEnum = (InputAndSingleOneFragmentFactoryMethod.FragmentStyleEnum) this.questionFragmentDataSource.getFragmentStyle();
			}
		} else {
